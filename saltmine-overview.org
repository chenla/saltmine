...
saltmine compiles to an archival programing language as well
as a binary
...
users never connect directly to the internet.
...
d9d knows who and where you are and builds a proxy profile
of false roles, it takes info from many different users to
create these roles which are used to access anything outside
of d9d.  this provides users with a plausible face to the
internet. 
